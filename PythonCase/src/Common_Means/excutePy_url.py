...
from Affiliate_test import Modify_Sourcing,Sourcing_Search
import logging
import sys
import thread
import dummy_thread
...
class StandardDialog(QDialog):  
    ...
    def __init__(self,parent=None):  
        ...
        self.setWindowTitle("Affiliate System")  
        ...
        newaffiliate=QPushButton(self.tr("Modify Affiliate"))  
        fontPushButton=QPushButton(self.tr("字体对话框"))  
        filePushButton01=QPushButton(self.tr("AffiliateLink测试按钮01")) 
        sourcingsearch=QPushButton(self.tr("Sourcing Search"))
  ...
  #      filePushButton.setFixedSize(QtCore.QSize(50,50))  
       ...
       # filePushButton.setStyleSheet("QPushButton{background-image:url(image/test03.jpg);background-repeat: repeat-xy;background-position: center;background-attachment: fixed;background-attachment: fixed;background-attachment: fixed;;background-clip: padding}");
        ...
        pausebutton.setStyleSheet("QPushButton{color:red;background:pink} QPushButton:hover{color:blue}") 
       # layout.addWidget(self.fileLineEdit,0,1)  
        ...
        layout.addWidget(sourcingsearch,1,1)  
        sourcingsearch.setStyleSheet("QPushButton{color:red;background:pink} QPushButton:hover{color:blue}") 
 #       layout.addWidget(self.fontLineEdit,2,1)  
      #  self.setStyle('''background-color:red;''')
        ...
        self.connect(sourcingsearch,SIGNAL("clicked()"),self.soursearch)  
        self.connect(newaffiliate, SIGNAL("clicked()"),self.modify)
        self.resize(500,500)
    ...
    def modify(self):
        thread.start_new_thread(Modify_Sourcing.modify_sorcing,())
    def soursearch(self):
        
        thread.start_new_thread(Sourcing_Search.sourcing_search,())
